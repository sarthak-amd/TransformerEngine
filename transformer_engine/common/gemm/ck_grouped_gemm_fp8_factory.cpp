...
std::unique_ptr<RunnerInterface> make_fp8_runner_typed(DType d_dtype, const GroupedGemmRunContext& ctx) {
...
std::unique_ptr<RunnerInterface> make_fp8_runner(DType a_dtype,
                                       DType b_dtype,
                                       DType d_dtype,
    ...
    switch (a_dtype) {
      case DType::kFloat8E4M3:
        if (b_dtype == DType::kFloat8E4M3) {
          return make_fp8_runner_typed<fp8e4m3, fp8e4m3, ALayout, BLayout>(d_dtype, ctx);
        } else if (b_dtype == DType::kFloat8E5M2) {
          return make_fp8_runner_typed<fp8e4m3, fp8e5m2, ALayout, BLayout>(d_dtype, ctx);
        }
        break;

      case DType::kFloat8E5M2:
        if (b_dtype == DType::kFloat8E4M3) {
          return make_fp8_runner_typed<fp8e5m2, fp8e4m3, ALayout, BLayout>(d_dtype, ctx);
        } else if (b_dtype == DType::kFloat8E5M2) {
          return make_fp8_runner_typed<fp8e5m2, fp8e5m2, ALayout, BLayout>(d_dtype, ctx);
        }
        break;
